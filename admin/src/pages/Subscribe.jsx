...
export default function Subscribe() {
  ...
  async function fetchList(p = page) {
    setLoading(true);
    try {
      const res = await api.get("/subscribe", { params: { page: p, limit: 10 } });
  ...
  async function handleDelete() {
    if (!selected) return;
    try {
      const res = await api.delete(`/subscribe/${selected.id}`);
      if (res.data && res.data.success) {
        fetchList();
  ...
  const actions = [
    { label: "View", onClick: (r) => { setSelected(r); setViewOpen(true); } },
  ...
  return (
    <div className="space-y-4">
      ...
      {/* View Modal */}
      <Modal isOpen={viewOpen} onClose={() => setViewOpen(false)} title="Subscription" data={selected} />
