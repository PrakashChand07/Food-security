...
export default function Visit() {
  ...
  async function fetchList(p = page) {
    setLoading(true);
    try {
      const res = await api.get("/visit", { params: { page: p, limit: 10 } });
  ...
  async function handleDelete() {
    if (!selected) return;
    try {
      const res = await api.delete(`/visit/${selected.id}`);
        if (res.data && res.data.success) {
        fetchList();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">Visit Registrations</h2>
      </div>
